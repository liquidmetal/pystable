...
class GaussianFilterTest(unittest.TestCase):
    ...
    def test_kernelGenerator(self):
        # Normalized
        kernel = stabilize.get_gaussian_kernel(1, 0, 0)
        self.assertEquals(kernel, [1])

        # Un normalized
        kernel = stabilize.get_gaussian_kernel(1, -1, 1)
    ...
    def test_filterOutput(self):
        output = stabilize.gaussian_filter(range(0, 1001))
    ...
    def test_diff(self):
        output = stabilize.diff([0, 1, 2, 3, 4])
